...
class TwitterScraper:
    ...
    def _is_rate_limited(self) -> bool:
        """A blunt substring match against the whole page isn't enough on its
        own: our marker phrases ("Try again", "Something went wrong") are
        generic enough to show up in ordinary tweet content too (e.g. a
        trader tweeting "trade went wrong, will try again tomorrow"), and a
        single false positive here used to be enough to wrongly reload the
        page and kill an entire hashtag's scrape early (see the stale-round
        reset below). A genuine rate-limit/anti-bot wall *replaces* the feed
        entirely, so requiring zero tweet articles alongside the marker text
        rules out the false-positive case while still catching the real one.
        """
        try:
            page_text = self.driver.page_source
        except WebDriverException:
            return False
        has_marker = any(marker.lower() in page_text.lower() for marker in selectors.RATE_LIMIT_MARKERS)
        if not has_marker:
            return False
        try:
            has_tweets = bool(self.driver.find_elements(By.CSS_SELECTOR, selectors.TWEET_ARTICLE))
        except WebDriverException:
            has_tweets = True  # fail open: a lookup error isn't evidence of a block
        return not has_tweets
    ...
    def search_hashtag(self, hashtag: str, target_count: int) -> Iterator[TweetRecord]:
        ...
        for iteration in range(max_rounds):
            ...
            if iteration % rate_limit_check_interval == 0 and self._is_rate_limited():
                ...
                self.driver.get(url)
                # A reload restarts the feed from the top, so the next few
                # rounds will legitimately re-show already-collected tweets
                # (filtered out as "not new") rather than genuinely stale
                # content - without this reset, that stretch alone could
                # trip the stale-round early-exit below and kill the run.
                stale_scroll_rounds = 0
                continue
            self.backoff.reset()
