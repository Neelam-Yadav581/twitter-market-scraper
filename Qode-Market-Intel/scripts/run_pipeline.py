...
def run(input_path: str, config_path: str = "config/config.yaml") -> None:
    ...
    if len(input_files) == 1:
        ...
        # Deterministic id for a given batch set, so re-running the same
        # combination of files overwrites the same parquet output rather
        # than accumulating a new file each time.
    ...
    plot_engagement_distribution(
        store.iter_batches(batch_size=500), reports_dir / "engagement_distribution.png"
    )
