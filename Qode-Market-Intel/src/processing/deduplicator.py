...
class Deduplicator:
    ...
    def __init__(
        ...
        # One bucket dict per band: a pair only needs to agree on ALL rows
        # within *some* band to become a comparison candidate, not on every
        # hash function at once. This OR-of-bands scheme (standard LSH
        # banding) trades a bit of extra bucket bookkeeping for much better
        # recall than a single big band - two tweets differing by one word
        # can still collide in at least one band even though their full
        # MinHash signatures differ.
        #
        # rows_per_band=1 (each band keyed by a single MinHash value) rather
        # than grouping rows into larger bands: for a true Jaccard similarity
        # s, a single band matches with probability s, so P(>=1 of 12 bands
        # matches) = 1-(1-s)^12. At our default threshold (s=0.6) that's
        # ~99.98% recall, vs. ~86% with 4 bands of 2 rows - the latter was
        # caught missing a genuine near-duplicate pair in testing. The cost
        # is a few more (cheap) false-positive candidates per lookup, which
        # the exact Jaccard check below filters out for free.
